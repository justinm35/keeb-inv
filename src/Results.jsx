...
const ResultsPage = ({results}) => {
  ...
  useEffect(()=> {
    if(Object.keys(results).length === 0){navigate('/')}
    ...
    return(
        ...
        <Content className="website-container">
          ...
          <Content className="results-container">
              {results.mino?.length > 0 ? results?.mino.map((x) => <ResultModule key={nanoid()} result={x}/>) :  <h3 className="website-title">Nothing Found</h3>}
          </Content>
        </Content>
        <Content className="website-container">
          <h2 className="website-title">KBD Fans<a className="website-title-link" href="kbdfans.com">kbdfans.com</a></h2>
          <Content className="results-container">
              {results.kbd?.length > 0 ? results?.kbd.map((x) => <ResultModule key={nanoid()} result={x}/>) : <h3 className="website-title">Nothing Found</h3>}
          </Content>
        </Content>
        <Content className="website-container">
          <h2 className="website-title">Apex Keyboards<a className="website-title-link" href="apexkeyboards.com">apexkeyboards.com</a></h2>
          <Content className="results-container">
              {results.apex?.length > 0 ? results?.apex.map((x) => <ResultModule key={nanoid()} result={x}/>) : <h3 className="website-title">Nothing Found</h3>}
          </Content>
        </Content>
      </Layout>
    )
